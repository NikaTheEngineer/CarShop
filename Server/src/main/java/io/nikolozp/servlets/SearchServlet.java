...
public class SearchServlet extends ParentServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (!isJSONValid(bodyStr)) {
            bodyStr = "{ '" + MyConstants.COMPANY + "' : '', '" + MyConstants.MODEL + "' : '' }";
        }
        ...
        if(wrongParamsForSearching(bodyObj)) {
            ...
            List<JSONObject> found = DbC.searchModel(bodyObj);
            if(found.size()!=0) {
    ...
    private boolean wrongParamsForSearching(JSONObject obj) {
        ...
        if(obj.has(MyConstants.MODEL) && !obj.get(MyConstants.MODEL).getClass().getName().equals("java.lang.String")) return true;
        if(!obj.has(MyConstants.COMPANY)) {
            obj.put(MyConstants.COMPANY, "");
        }
        if(!obj.has(MyConstants.MODEL)) {
            obj.put(MyConstants.MODEL, "");
        }
        return false;
    }
